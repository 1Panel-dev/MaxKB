...
def _get_authorized_resource_query_set(auth, user_id, model, auth_target_type, read_permission, workspace_id=None):
    """返回当前用户授权可访问的资源 QuerySet。
    workspace_id 缺省 => 跨所有授权工作空间（全局）；给出 => 仅该工作空间"""
    permission_list = _PERM_WITH_ROLE if hasPermission(auth, read_permission) else _PERM_DEFAULT
    if workspace_id:
        # 指定工作空间：复用现有单工作空间授权逻辑
        ...
        if is_extends_workspace_manage(auth, workspace_id):
            if has_extends_workspace_manage_permission(auth, read_permission, workspace_id):
                return QuerySet(model).filter(workspace_id=workspace_id)
        if not has_all_permission(auth, read_permission, workspace_id):
    ...
    manage_ws_ids = [
        w["id"]
        for w in get_workspace_list_by_user(user_id)
        if is_workspace_manage(auth, w["id"])
        or (
            is_extends_workspace_manage(auth, w["id"])
            and has_extends_workspace_manage_permission(auth, read_permission, w["id"])
        )
    ]
    perm_ids = (
        QuerySet(WorkspaceUserResourcePermission)
        .filter(user_id=user_id, auth_target_type=auth_target_type, permission_list__overlap=permission_list)
        .exclude(target="default")
        .annotate(target_uuid=Cast("target", output_field=UUIDField()))
        .values_list("target_uuid", flat=True)
    )
    return QuerySet(model).filter(Q(workspace_id__in=manage_ws_ids) | Q(id__in=perm_ids))


def _get_authorized_application_query_set(auth, user_id, workspace_id=None):
    return _get_authorized_resource_query_set(
        auth, user_id, Application, "APPLICATION", PermissionConstants.APPLICATION_READ.value, workspace_id
    )


class SystemHomePageSerializer(serializers.Serializer):
    """系统管理首页：跨所有授权工作空间的资源数据（不绑定单个工作空间）"""
    ...
    class Application(serializers.Serializer):
        ...
        def aggregation(self, auth, with_valid=True):
            ...
            query_set = _get_authorized_application_query_set(auth, self.data["user_id"], self.data.get("workspace_id"))
    ...
    class Knowledge(serializers.Serializer):
        ...
        def aggregation(self, auth, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            query_set = _get_authorized_resource_query_set(
                auth,
                self.data["user_id"],
                Knowledge,
                "KNOWLEDGE",
                PermissionConstants.KNOWLEDGE_READ.value,
            ...
            result = query_set.aggregate(
                ...
                failure_count=Count(
                    "document",
                    filter=Q(document__status__contains="3"),
                    distinct=True,
                ),
            )
            return {
    ...
    class Tool(serializers.Serializer):
        ...
        def aggregation(self, auth, with_valid=True):
            ...
            query_set = _get_authorized_resource_query_set(
                ...
                Tool,
                "TOOL",
                PermissionConstants.TOOL_READ.value,
                self.data.get("workspace_id"),
            )
            result = query_set.aggregate(
                total=Count("id"),
    ...
    class Model(serializers.Serializer):
        ...
        def aggregation(self, auth, with_valid=True):
            ...
            query_set = _get_authorized_resource_query_set(
                ...
                Model,
                "MODEL",
                PermissionConstants.MODEL_READ.value,
                self.data.get("workspace_id"),
            )
            result = query_set.aggregate(
                total=Count("id"),
    ...
    class TokensAggregation(serializers.Serializer):
        ...
        def aggregation(self, auth, with_valid=True):
            ...
            query = ChatRecord.objects.filter(
                create_time__gte=get_start_time(data["start_time"]),
                create_time__lte=get_end_time(data["end_time"]),
    ...
    class ApplicationTokensRanking(serializers.Serializer):
        ...
        def get_queryset(self, auth, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            data = self.data
            record_time_filter = (
                Q(chat__is_deleted=False)
                & Q(chat__chatrecord__create_time__gte=get_start_time(data["start_time"]))
                & Q(chat__chatrecord__create_time__lte=get_end_time(data["end_time"]))
            )
            token_expr = ExpressionWrapper(
                F("chat__chatrecord__message_tokens") + F("chat__chatrecord__answer_tokens"),
                output_field=BigIntegerField(),
            )
            queryset = _get_authorized_application_query_set(auth, data["user_id"], data.get("workspace_id"))
            if data.get("name"):
    ...
    class ApplicationQuestionRanking(serializers.Serializer):
        ...
        def get_queryset(self, auth, with_valid=True):
            ...
            return queryset.annotate(
                chat_record_count_total=Coalesce(
                    Count("chat__chatrecord__id", filter=record_time_filter), Value(0), output_field=BigIntegerField()
                ),
                chat_user_count=Count(
                    "chat__chat_user_id",
                    filter=(record_time_filter & Q(chat__chat_user_id__isnull=False) & ~Q(chat__chat_user_id="")),
        ...
        def export(self, auth, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            chat_record_number = SystemHomePageSerializer.ChatRecordAggregation(data=self.data).aggregation(auth)[
                "total_count"
            ]
            queryset = self.get_queryset(auth, with_valid=False)
            workbook = openpyxl.Workbook(write_only=True)
    ...
    class ApplicationUserTokenRanking(serializers.Serializer):
        ...
        def get_queryset(self, auth, with_valid=True):
            ...
            base_queryset = base_queryset.filter(
                application_id__in=_get_authorized_application_query_set(
                    auth, data["user_id"], data.get("workspace_id")
                )
            )
    ...
    class ApplicationMonitoring(serializers.Serializer):
        ...
        def get_chat_record_aggregate_trend(self, auth, with_valid=True):
            ...
            application_query_set = _get_authorized_application_query_set(
                auth, data["user_id"], data.get("workspace_id")
            )
