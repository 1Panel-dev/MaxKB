...
class ChatRecordLinkView(APIView):
    ...
    def post(self, request: Request, application_id: str, chat_id: str):
        return result.success(
            ChatRecordShareLinkSerializer(
                data={"application_id": application_id, "chat_id": chat_id, "user_id": request.user.id}
...
class ChatRecordDetailView(APIView):
    ...
    def get(self, request, link: str):
        return result.success(ChatShareLinkDetailSerializer(data={"link": link}).get_record_list())
