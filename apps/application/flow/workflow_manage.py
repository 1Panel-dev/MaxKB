...
class WorkflowManage:
    def __init__(self, flow: Flow, params, work_flow_post_handler: WorkFlowPostHandler,
                 base_to_response: BaseToResponse = SystemToResponse(), form_data=None, start_node_id=None,
                 start_node_data=None, chat_record_details=None):
        ...
        self.chat_record_details = chat_record_details
        if start_node_id is not None:
            self.load_node(chat_record_details, start_node_id, start_node_data)
        else:
            self.node_context = []

    def load_node(self, chat_record_details, start_node_id, start_node_data):
        self.node_context = []
        for node_details in sorted(chat_record_details.values(), key=lambda d: d.get('index')):
            ...
            if node_details.get('runtime_node_id') == start_node_id:
                ...
                node_result = NodeResult({**start_node_data, 'form_data': start_node_data}, {})
    ...
    def get_runtime_details(self):
        ...
        for index in range(len(self.node_context)):
            ...
            if self.chat_record_details is not None:
                details = self.chat_record_details.get(node.runtime_node_id)
