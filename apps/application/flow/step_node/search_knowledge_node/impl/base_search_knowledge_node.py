...
def get_none_result(question):
    return NodeResult(
        {"paragraph_list": [], "is_hit_handling_method": [], "question": question, "data": "", "directly_return": ""},
        {},
    )
...
class BaseSearchKnowledgeNode(ISearchKnowledgeStepNode):
    def save_context(self, details, workflow_manage):
        ...
        self.context["data"] = "\n".join(
            [f"{paragraph.get('title', '')}:{paragraph.get('content')}" for paragraph in result]
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not self.workflow_manage.get_body().get("debug", False):
        ...
        return NodeResult(
            {
                ...
                "data": "\n".join(
                    [f"{reset_title(paragraph.get('title', ''))}{paragraph.get('content')}" for paragraph in result]
                )[0 : knowledge_setting.get("max_paragraph_char_number", 5000)],
                "directly_return": "\n".join(
