...
class IntentNode(INode):
    ...
    def execute(self):
        ...
        user_input = self.workflow_manage.get_reference_field(content_list_ref[0], content_list_ref[1:])
    ...
    def _build_classification_prompt(self, user_input: str, branch: List[Dict]) -> str:
        ...
        other_branch = self._find_other_branch(branch)
        if other_branch:
            classification_list.append({"classificationId": 0, "content": other_branch.get("content")})
        classification_id = 1
        for b in branch:
            if not b.get("isOther"):
                classification_list.append({"classificationId": classification_id, "content": b["content"]})
                classification_id += 1

        return PROMPT_TEMPLATE.format(classification_list=classification_list, user_input=user_input)
    ...
    def get_details(self, index: int = 0, position: dict = None, old_details: dict = None, **kwargs):
        ...
        details.update(
            {
                "user_input": self.get_context("user_input"),
                "answer": self.get_context("answer"),
                "branch_id": self.get_context("branch_id"),
                "category": self.get_context("category"),
                "reason": self.get_context("reason"),
