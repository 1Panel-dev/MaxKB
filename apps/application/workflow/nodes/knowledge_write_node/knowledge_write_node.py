...
@project: MaxKB
@Author:  虎虎虎
@file:    knowledge_write_node.py
@date:    2026/9/11
@desc:    知识库写入节点：把上游产出的文档/段落写入知识库并触发向量化
"""
...
class KnowledgeWriteNodeParamSerializer(serializers.Serializer):
    document_list = serializers.ListField(
        required=True, child=serializers.CharField(required=True), allow_null=True, label=_("document list")
    )
...
class KnowledgeWriteNode(INode):
    serializer_class = KnowledgeWriteNodeParamSerializer
    ...
    def execute(self):
        ...
        self.write_context("write_content", write_content_list)
    ...
    def get_details(self, index: int = 0, position: dict = None, old_details: dict = None, **kwargs):
        ...
        details.update(
            {
                "write_content": self.get_context("write_content"),
