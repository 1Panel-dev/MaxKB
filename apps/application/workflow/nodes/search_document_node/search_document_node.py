...
def _handle_auto_tags(workflow_manage, document_id_list, question_reference):
    question = workflow_manage.get_reference_field(question_reference[0], question_reference[1:]) if question_reference else ''
...
class SearchDocumentNode(INode):
    ...
    def execute(self):
        ...
        if search_scope_type == 'custom':
            ...
            document_id_list = list(QuerySet(Document).filter(
                knowledge_id__in=knowledge_id_list
            ).values_list('id', flat=True))
        else:
            if search_scope_source == 'document':
                document_id_list = self.workflow_manage.get_reference_field(
                    search_scope_reference[0], search_scope_reference[1:]
                ) if search_scope_reference else []
            else:
                ref_knowledge_ids = self.workflow_manage.get_reference_field(
                    search_scope_reference[0], search_scope_reference[1:]
                ) if search_scope_reference else []
        ...
        if get_knowledge_list_of_authorized is not None and RoleConstants.CHAT_USER.value.name == chat_user_type:
            ...
            authorized_knowledge_ids = get_knowledge_list_of_authorized(
                workflow_params.get('chat_user_id'),
