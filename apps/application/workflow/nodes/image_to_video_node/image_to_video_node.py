...
class ImageToVideoNode(INode):
    ...
    def execute(self):
        ...
        if model_id_type == "default":
            default_model_setting = workflow_params.get("default_model_setting").get("ITV") or {}
        ...
        first_frame_url = self.workflow_manage.get_reference_field(first_frame_url_ref[0], first_frame_url_ref[1:])

        last_frame_url = None
        if last_frame_url_ref is not None and last_frame_url_ref != []:
            last_frame_url = self.workflow_manage.get_reference_field(last_frame_url_ref[0], last_frame_url_ref[1:])

        ttv_model = get_model_instance_by_model_workspace_id(model_id, workspace_id, **(model_params_setting or {}))
    ...
    def _upload_knowledge_file(self, file, workflow_params):
        ...
        file_url = FileSerializer(
            data={"file": file, "meta": meta, "source_id": knowledge_id, "source_type": FileSourceType.KNOWLEDGE.value}
        ).upload()
        return file_url
    ...
    def _upload_tool_file(self, file, workflow_params):
        ...
        file_url = FileSerializer(
            data={"file": file, "meta": meta, "source_id": tool_id, "source_type": FileSourceType.TOOL.value}
        ).upload()
        return file_url
        ...
        details.update(
            {
                ...
                "first_frame_url": self.get_context("first_frame_url"),
                "last_frame_url": self.get_context("last_frame_url"),
