...
class RerankerNode(INode):
    ...
    def execute(self):
        ...
        reranker_list = [self.workflow_manage.get_reference_field(ref[0], ref[1:])
                         for ref in reranker_reference_list]

        if reranker_model_id_type == 'reference' and reranker_model_id_reference:
            reference_data = self.workflow_manage.get_reference_field(
                reranker_model_id_reference[0], reranker_model_id_reference[1:],
            )
        ...
        self.write_context('is_hit_handling_method_list',
                           [row for row in r if row.get('metadata').get('is_hit_handling_method')])
