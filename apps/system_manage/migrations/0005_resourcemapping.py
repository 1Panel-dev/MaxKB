...
def get_initialization_resource_mapping():
    ...
    for app_id in ids:
        try:
            ...
            workflow_mapping = get_workflow_resource(
                application.work_flow, get_node_handle_callback(ResourceType.APPLICATION, application.id)
            )
            ...
            if knowledge.type == 4:
                ...
                if knowledge_workflow:
                    workflow_mapping = get_workflow_resource(
                        ...
                        get_node_handle_callback(ResourceType.KNOWLEDGE, str(knowledge_workflow.knowledge_id)),
                    )
                    resource_mapping_list += workflow_mapping
            instance_mapping = get_instance_resource(
    ...
    return {
        (str(item.target_type) + str(item.target_id) + str(item.source_type) + str(item.source_id)): item
