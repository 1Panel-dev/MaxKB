...
这些函数只解析 work_flow 的 JSON 结构(nodes/properties/node_data)或实例字段,
据此推导它引用的目标资源并登记到 system_manage.ResourceMapping。与执行引擎无关,
故归属于 ResourceMapping 所在的 system_manage,不放在工作流引擎(flow / workflow)里。
"""
