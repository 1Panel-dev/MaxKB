...
class ChatUserInstanceSerializer(serializers.ModelSerializer):
    class Meta:
        model = ChatUser
        fields = ['id', 'username', 'email', 'phone', 'is_active', 'nick_name', 'create_time', 'update_time',
                  'source']
...
def add_or_edit_user_group_relation(user, user_group_ids):
    ...
    UserGroupRelation.objects.bulk_create([
        UserGroupRelation(user=user, group=group)
        for group in groups
    ])
...
class ChatUserSerializer(serializers.Serializer):
    ...
    class BatchAddGroup(serializers.Serializer):
        ...
        is_append = serializers.BooleanField(
            required=False,
            label=_('Is Append'),
            default=False
        )
        ...
        def batch_add_group(self, with_valid=True):
            ...
            if is_append:
                # 获取现有关系
                existing_relations = UserGroupRelation.objects.filter(
    ...
    class Operate(serializers.Serializer):
        ...
        def edit(self, instance, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                ChatUserSerializer.UserEditInstance(data=instance).is_valid(user_id=self.data.get('id'),
...
class UserGroupAddMemberSerializer(serializers.Serializer):
    ...
    def add_member(self, with_valid=True):
        ...
        current_user_group_ids = set(
            ...
            UserGroupRelation.objects.filter(group__id=self.data.get('id')).values_list('user_id', flat=True)
        )
...
class RePasswordSerializer(serializers.Serializer):
    ...
    def reset_password(self, user_id):
        ...
        """
        if self.is_valid():
            QuerySet(ChatUser).filter(id=user_id).update(
