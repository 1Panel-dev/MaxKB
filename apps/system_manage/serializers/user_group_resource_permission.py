    ...
    def get_queryset(self, instance):
        ...
        return {
            ...
            'workspace_user_group_resource_permission_query_set': QuerySet(WorkspaceUserGroupResourcePermission).filter(
                workspace_id=self.data.get('workspace_id'), user_group_id=self.data.get('user_group_id'),
    ...
    def edit(self, instance, user, with_valid=True):
        ...
        for user_resource_permission in instance:
            ...
            if len(exist_list) > 0:
                ...
                save_list.append(WorkspaceUserGroupResourcePermission(target=user_resource_permission.get('target_id'),
                                                                 auth_target_type=self.data.get('auth_target_type'),
                                                                 ...
                                                                 auth_type=auth_type))
        # 批量更新
        QuerySet(WorkspaceUserGroupResourcePermission).bulk_update(update_list,
                                                                   ['permission_list', 'auth_type']) if len(
            update_list) > 0 else None
        # 批量插入
        QuerySet(WorkspaceUserGroupResourcePermission).bulk_create(save_list) if len(save_list) > 0 else None
        version = Cache_Version.PERMISSION_LIST.get_version()
        key = Cache_Version.PERMISSION_LIST.get_key(user_group_id=user_group_id)
        cache.delete(key, version=version)
        return instance
...
class ResourceUserGroupPermissionSerializer(serializers.Serializer):
    ...
    def list(self, instance, with_valid=True):
        ...
        resource_user_permission_list = native_search(self.get_queryset(instance), get_file_content(
            os.path.join(PROJECT_DIR, "apps", "system_manage",
                         'sql',
                         'get_resource_user_group_permission_detail.sql'
                         )
        ))
    ...
    def get_has_manage_permission_resource_under_folders(self, current_user_id, folder_ids):
        ...
        current_user_managed_resources_ids = QuerySet(WorkspaceUserGroupResourcePermission).filter(
