...
model_info_list = [
    ModelInfo("Phi-4", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
    ModelInfo("DeepSeek-R1-Distill-Qwen-32B", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
    ModelInfo("maestrale-chat-v0.4-beta", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
    ModelInfo("Llama-3.3-70B-Instruct", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
    ModelInfo("Llama-3.1-8B-Instruct", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
    ModelInfo("DeepSeek-Coder-6.7B-Instruct", "", ModelTypeConst.LLM, openai_llm_model_credential, RegoloChatModel),
]
open_ai_embedding_credential = RegoloEmbeddingCredential()
model_info_embedding_list = [
    ModelInfo("gte-Qwen2", "", ModelTypeConst.EMBEDDING, open_ai_embedding_credential, RegoloEmbeddingModel),
]

model_info_tti_list = [
    ModelInfo("FLUX.1-dev", "", ModelTypeConst.TTI, openai_tti_model_credential, RegoloTextToImage),
    ModelInfo("sdxl-turbo", "", ModelTypeConst.TTI, openai_tti_model_credential, RegoloTextToImage),
]
model_info_manage = (
    ...
    def get_model_provide_info(self):
        return ModelProvideInfo(
            ...
            icon=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "models_provider", "impl", "regolo_model_provider", "icon", "regolo_icon_svg"
                )
            ),
        )
