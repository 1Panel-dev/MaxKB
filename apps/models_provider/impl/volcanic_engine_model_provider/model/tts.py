...
class VolcanicEngineTextToSpeech(MaxKBBaseModel, BaseTextToSpeech):
    ...
    def text_to_speech(self, text):
        request_json = {
            "app": {"appid": self.volcanic_app_id, "token": "access_token", "cluster": self.volcanic_cluster},
            "user": {"uid": "uid"},
            "audio": {
                "encoding": "mp3",
                "volume_ratio": 1.0,
            ...
            | self.params,
            "request": {"reqid": str(uuid.uuid7()), "text": "", "text_type": "plain", "operation": "xxx"},
        }
    ...
    async def submit(self, request_json, text):
        ...
        async with websockets.connect(
            self.volcanic_api_url, additional_headers=header, ping_interval=None, ssl=ssl_context
        ) as ws:
