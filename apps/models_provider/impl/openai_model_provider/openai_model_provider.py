...
model_info_list = [
    ...
    ModelInfo(
        "gpt-4o",
        _("The latest GPT-4o, cheaper and faster than gpt-4-turbo, updated with OpenAI adjustments"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ModelInfo(
        "gpt-4o-mini",
        _("The latest gpt-4o-mini, cheaper and faster than gpt-4o, updated with OpenAI adjustments"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ...
    ModelInfo(
        "gpt-3.5-turbo-0125",
        _("gpt-3.5-turbo snapshot on January 25, 2024, supporting context length 16,385 tokens"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ModelInfo(
        "gpt-3.5-turbo-1106",
        _("gpt-3.5-turbo snapshot on November 6, 2023, supporting context length 16,385 tokens"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ...
    ModelInfo(
        "gpt-4-0125-preview",
        _("gpt-4-turbo snapshot on January 25, 2024, supporting context length 128,000 tokens"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ModelInfo(
        "gpt-4-1106-preview",
        _("gpt-4-turbo snapshot on November 6, 2023, supporting context length 128,000 tokens"),
        ModelTypeConst.LLM,
        openai_llm_model_credential,
        OpenAIChatModel,
    ),
    ModelInfo("whisper-1", "", ModelTypeConst.STT, openai_stt_model_credential, OpenAISpeechToText),
    ModelInfo("tts-1", "", ModelTypeConst.TTS, openai_tts_model_credential, OpenAITextToSpeech),
]
open_ai_embedding_credential = OpenAIEmbeddingCredential()
model_info_embedding_list = [
    ModelInfo(
        "text-embedding-ada-002", "", ModelTypeConst.EMBEDDING, open_ai_embedding_credential, OpenAIEmbeddingModel
    ),
    ModelInfo(
        "text-embedding-3-small", "", ModelTypeConst.EMBEDDING, open_ai_embedding_credential, OpenAIEmbeddingModel
    ),
    ModelInfo(
        "text-embedding-3-large", "", ModelTypeConst.EMBEDDING, open_ai_embedding_credential, OpenAIEmbeddingModel
    ),
]
...
model_info_tti_list = [
    ModelInfo("dall-e-3", "", ModelTypeConst.TTI, openai_tti_model_credential, OpenAITextToImage),
]
    ...
    def get_model_provide_info(self):
        return ModelProvideInfo(
            ...
            icon=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "models_provider", "impl", "openai_model_provider", "icon", "openai_icon_svg"
                )
            ),
        )
