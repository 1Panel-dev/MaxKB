...
model_info_list = [
    ...
    ModelInfo(
        "BAAI/Aquila-7B", _("BAAI’s 7B parameter model"), ModelTypeConst.LLM, v_llm_model_credential, VllmChatModel
    ),
    ModelInfo(
        "BAAI/AquilaChat-7B", _("BAAI’s 13B parameter mode"), ModelTypeConst.LLM, v_llm_model_credential, VllmChatModel
    ),
]
    ...
    def get_model_provide_info(self):
        return ModelProvideInfo(
            ...
            icon=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "models_provider", "impl", "vllm_model_provider", "icon", "vllm_icon_svg"
                )
            ),
        )
