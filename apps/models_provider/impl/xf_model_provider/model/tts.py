...
class XFSparkTextToSpeech(MaxKBBaseModel, BaseTextToSpeech):
    ...
    def new_instance(model_type, model_name, model_credential: Dict[str, object], **model_kwargs):
        ...
        params = {'api_version': api_version, 'vcn': vcn}
    ...
    async def send(self, ws, text):
        api_version = None
        if self.params and 'api_version' in self.params:
            api_version = self.params.get('api_version')
        else:
            api_version = 'super_humanoid' if '/v1/private/' in (self.spark_api_url or '') else 'online'

        vcn_value = self.params.get("vcn", None)
