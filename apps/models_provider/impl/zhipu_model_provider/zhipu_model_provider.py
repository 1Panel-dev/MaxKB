...
model_info_image_list = [
    ...
    ModelInfo(
        "glm-4v",
        _("Focus on single picture understanding. Suitable for scenarios requiring efficient image analysis"),
        ModelTypeConst.IMAGE,
        zhipu_image_model_credential,
        ZhiPuImage,
    ),
    ModelInfo(
        "glm-4v-flash",
        _("Focus on single picture understanding. Suitable for scenarios requiring efficient image analysis (free)"),
        ModelTypeConst.IMAGE,
        zhipu_image_model_credential,
        ZhiPuImage,
    ),
...
model_info_tti_list = [
    ModelInfo(
        "cogview-3",
        _("Quickly and accurately generate images based on user text descriptions. Resolution supports 1024x1024"),
        ModelTypeConst.TTI,
    ...
    ModelInfo(
        "cogview-3-plus",
        _("Generate high-quality images based on user text descriptions, supporting multiple image sizes"),
        ModelTypeConst.TTI,
        zhipu_tti_model_credential,
        ZhiPuTextToImage,
    ),
    ModelInfo(
        "cogview-3-flash",
        _("Generate high-quality images based on user text descriptions, supporting multiple image sizes (free)"),
        ModelTypeConst.TTI,
        zhipu_tti_model_credential,
        ZhiPuTextToImage,
    ),
    ...
    def get_model_provide_info(self):
        return ModelProvideInfo(
            ...
            icon=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "models_provider", "impl", "zhipu_model_provider", "icon", "zhipuai_icon_svg"
                )
            ),
        )
