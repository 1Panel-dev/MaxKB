...
def get_edit_model_details(request):
    ...
    return {"path": path, "body": {**body, "credential": credential_encryption_ed}, "query": query}
...
class ModelSetting(APIView):
    ...
    def post(self, request: Request, workspace_id: str):
        return result.success(
            ModelSerializer.Create(
                data={**request.data, "user_id": request.user.id, "workspace_id": workspace_id}
    ...
    class ModelMeta(APIView):
        ...
        @extend_schema(
            methods=["GET"],
            summary=_("Query model meta information, this interface does not carry authentication information"),
            description=_("Query model meta information, this interface does not carry authentication information"),
            operation_id=_("Query model meta information, this interface does not carry authentication information"),  # type: ignore
            parameters=GetModelApi.get_parameters(),
            responses=GetModelApi.get_response(),
            tags=[_("Model")],  # type: ignore
        )
