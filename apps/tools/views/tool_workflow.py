...
from tools.serializers.tool_workflow import StoreToolWorkflow, ToolWorkflowMcpSerializer, ToolWorkflowSerializer
...
class ToolWorkflowView(APIView):
    ...
    class Publish(APIView):
        ...
        @has_permissions(
            PermissionConstants.TOOL_PUBLISH.get_workspace_tool_permission(),
...
class ToolWorkflowDebugView(APIView):
    ...
    def post(self, request: Request, workspace_id: str, tool_id: str):
        return ToolWorkflowSerializer.Operate(
            data={"workspace_id": workspace_id, "tool_id": tool_id, "user_id": request.user.id}
        ).debug(request.data, request.user, True)
