...
from knowledge.services.problem_service import save_problem
...
def generate_problem_by_paragraph(paragraph, llm_model, prompt):
    try:
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph.id), TaskType.GENERATE_PROBLEM, State.STARTED
        )
        ...
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph.id), TaskType.GENERATE_PROBLEM, State.SUCCESS
        )
    except Exception as e:
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph.id), TaskType.GENERATE_PROBLEM, State.FAILURE
        )
...
@celery_app.task(base=QueueOnce, once={"keys": ["knowledge_id"]}, name="celery:generate_related_by_knowledge")
...
@celery_app.task(base=QueueOnce, once={"keys": ["document_id"]}, name="celery:generate_related_by_document")
def generate_related_by_document_id(document_id, model_id, model_params_setting, prompt, state_list=None):
    ...
    try:
        ...
        if is_the_task_interrupted():
            return
        ListenerManagement.update_status(
            QuerySet(Document).filter(id=document_id), TaskType.GENERATE_PROBLEM, State.STARTED
        )
        llm_model = get_llm_model(model_id, model_params_setting)

        # 生成问题函数
        generate_problem = get_generate_problem(
            llm_model, prompt, ListenerManagement.get_aggregation_document_status(document_id), is_the_task_interrupted
        )
        query_set = (
            QuerySet(Paragraph)
            .annotate(
        ...
        maxkb_logger.error(
            _("Generate issue based on document: {document_id} error {error}{traceback}").format(
...
@celery_app.task(base=QueueOnce, once={"keys": ["paragraph_id_list"]}, name="celery:generate_related_by_paragraph_list")
def generate_related_by_paragraph_id_list(document_id, paragraph_id_list, model_id, model_params_setting, prompt):
    try:
        is_the_task_interrupted = get_is_the_task_interrupted(document_id)
        if is_the_task_interrupted():
            ListenerManagement.update_status(
                QuerySet(Document).filter(id=document_id), TaskType.GENERATE_PROBLEM, State.REVOKED
            )
            return
        ListenerManagement.update_status(
            QuerySet(Document).filter(id=document_id), TaskType.GENERATE_PROBLEM, State.STARTED
        ...
        # 生成问题函数
        generate_problem = get_generate_problem(
            llm_model, prompt, ListenerManagement.get_aggregation_document_status(document_id)
        )
