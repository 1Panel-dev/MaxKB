...
class ProblemBatchDeleteSerializer(serializers.Serializer):
    problem_id_list = serializers.ListField(
        required=True, label=_("problem id list"), child=serializers.UUIDField(required=True, label=_("problem id"))
    )
    ...
    class Operate(serializers.Serializer):
        ...
        def list_paragraph(self, with_valid=True):
            ...
            problem_paragraph_mapping = QuerySet(ProblemParagraphMapping).filter(
                knowledge_id=self.data.get("knowledge_id"), problem_id=self.data.get("problem_id")
            )
        ...
        def one(self, with_valid=True):
            ...
            return ProblemInstanceSerializer(
                QuerySet(Problem).get(id=self.data.get("problem_id"), knowledge_id=self.data.get("knowledge_id"))
            ).data
    ...
    class Create(serializers.Serializer):
        ...
        def batch(self, problem_list, with_valid=True):
            ...
            exists_problem_content_list = [
                problem.content
                for problem in QuerySet(Problem).filter(knowledge_id=knowledge_id, content__in=problem_list)
            ]
            problem_instance_list = [
                Problem(id=uuid.uuid7(), knowledge_id=knowledge_id, content=problem_content)
...
def is_exits(exits_problem_paragraph_mapping_list, new_paragraph_mapping):
    filter_list = [
        ...
        if str(exits_problem_paragraph_mapping.paragraph_id) == new_paragraph_mapping.paragraph_id
