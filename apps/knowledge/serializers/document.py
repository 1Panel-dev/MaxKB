...
class DocumentEditInstanceSerializer(serializers.Serializer):
    ...
    name = serializers.CharField(
        required=False, max_length=128, min_length=1, label=_("document name"), source=_("document name")
    )
...
class DocumentSerializers(serializers.Serializer):
    class Export(serializers.Serializer):
        type = serializers.CharField(
            ...
            validators=[
                validators.RegexValidator(
                    regex=re.compile("^csv|excel$"), message=_("The template type only supports excel|csv"), code=500
                )
            ],
            label=_("type"),
        )
    ...
    class Migrate(serializers.Serializer):
        ...
        document_id_list = serializers.ListField(
            required=True, label=_("document list"), child=serializers.UUIDField(required=True, label=_("document id"))
        )
        ...
        def migrate(self, with_valid=True):
            ...
            problem_list = QuerySet(Problem).filter(
                id__in=[
                    problem_paragraph_mapping.problem_id for problem_paragraph_mapping in problem_paragraph_mapping_list
                ]
            )
            ...
            target_handle_problem_list = [
                self.get_target_knowledge_problem(
                    target_knowledge_id, problem_paragraph_mapping, problem_list, target_problem_list
                )
            ...
            if model_id:
                ...
                ListenerManagement.update_status(
                    QuerySet(Document).filter(id__in=document_id_list), TaskType.EMBEDDING, State.PENDING
                )
                ListenerManagement.update_status(
                    QuerySet(Paragraph).filter(document_id__in=document_id_list), TaskType.EMBEDDING, State.PENDING
                )
        ...
        def get_target_knowledge_problem(
            target_knowledge_id: str, problem_paragraph_mapping, source_problem_list, target_problem_list
        ):
            source_problem_list = [
                source_problem
    ...
    class Query(serializers.Serializer):
        ...
        tag_ids = serializers.ListField(
            child=serializers.UUIDField(), allow_null=True, required=False, allow_empty=True
        )
        ...
        def get_query_set(self):
            ...
            if no_tag and tag_ids:
                ...
                query_set = query_set.filter(Q(id__in=matched_doc_ids) | ~Q(id__in=tagged_doc_ids))
            elif no_tag:
                tagged_doc_ids = QuerySet(DocumentTag).values_list("document_id", flat=True)
            ...
            if "tag" in self.data and self.data.get("tag") not in [None, ""]:
                ...
                document_id_list = (
                    ...
                    .filter(Q(tag__key__icontains=tag_name) | Q(tag__value__icontains=tag_name))
            ...
            return {"document_custom_sql": query_set, "order_by_query": order_by_query_set}
    ...
    class Operate(serializers.Serializer):
        ...
        def export(self, with_valid=True):
            ...
            paragraph_query_set = (
                QuerySet(Paragraph).filter(document_id=self.data.get("document_id")).order_by("position")
            )
            ...
            problem_mapping_list = native_search(
                QuerySet(ProblemParagraphMapping).filter(document_id=self.data.get("document_id")),
        ...
        def one(self, with_valid=False):
            ...
            return native_search(
                {"document_custom_sql": query_set, "order_by_query": QuerySet(Document).order_by("-create_time", "id")},
                select_string=get_file_content(
                    os.path.join(PROJECT_DIR, "apps", "knowledge", "sql", "list_document.sql")
                ),
    ...
    class Create(serializers.Serializer):
        ...
        def post_embedding(result, document_id, knowledge_id):
            DocumentSerializers.Operate(data={"knowledge_id": knowledge_id, "document_id": document_id}).refresh()
            return result
        ...
        def get_paragraph_model(document_model, paragraph_list: List):
            ...
            paragraph_model_dict_list = [
                ParagraphSerializers.Create(
                    data={"knowledge_id": knowledge_id, "document_id": str(document_model.id)}
    ...
    class BatchGenerateRelated(serializers.Serializer):
        ...
        def batch_generate_related(self, instance: Dict, with_valid=True):
            ...
            state_list = instance.get("state_list")
            ListenerManagement.update_status(
                QuerySet(Document).filter(id__in=document_id_list), TaskType.GENERATE_PROBLEM, State.PENDING
            )
            ListenerManagement.update_status(
                QuerySet(Paragraph)
                .annotate(
    ...
    class Tags(serializers.Serializer):
        ...
        def list(self):
            ...
            tag_ids = (
                QuerySet(DocumentTag).filter(document_id=self.data.get("document_id")).values_list("tag_id", flat=True)
            )
            ...
            if self.data.get("name"):
                tag_ids = (
                    ...
                    .filter(Q(key__icontains=self.data.get("name")) | Q(value__icontains=self.data.get("name")))
    ...
    class DeleteTags(serializers.Serializer):
        ...
        def delete_tags(self):
            ...
            document_id = self.data.get("document_id")
            tag_ids = self.data.get("tag_ids")
            QuerySet(DocumentTag).filter(document_id=document_id, tag_id__in=tag_ids).delete()
    ...
    class DeleteDocsTag(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                not QuerySet(Tag)
                .filter(id=self.data.get("tag_id"), knowledge_id=self.data.get("knowledge_id"))
                .exists()
            ):
