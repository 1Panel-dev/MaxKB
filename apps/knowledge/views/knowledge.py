...
from knowledge.serializers.knowledge_sync import (
    KnowledgeSyncLogQuerySerializer,
...
class KnowledgeView(APIView):
    ...
    class BatchDelete(APIView):
        ...
        def put(self, request: Request, workspace_id: str):
            ...
            @log(
                menu="Knowledge Base",
                operate="Batch delete knowledge",
    ...
    class KnowledgeSyncSetting(APIView):
        authentication_classes = [TokenAuth]

        @extend_schema(
            methods=["GET"],
            summary=_("Get knowledge scheduled synchronization setting"),
            description=_("Get knowledge scheduled synchronization setting"),
            operation_id=_("Get knowledge scheduled synchronization setting"),  # type: ignore
            parameters=KnowledgeSyncSettingAPI.get_parameters()[:2],
            responses=KnowledgeSyncSettingAPI.get_response(),
            tags=[_("Knowledge Base")],  # type: ignore
        )
        @has_permissions(
            PermissionConstants.KNOWLEDGE_READ.get_workspace_knowledge_permission(),
        ...
        @extend_schema(
            methods=["PUT"],
            summary=_("Update knowledge scheduled synchronization setting"),
            description=_("Update knowledge scheduled synchronization setting"),
            operation_id=_("Update knowledge scheduled synchronization setting"),  # type: ignore
            parameters=KnowledgeSyncSettingAPI.get_parameters()[:2],
            request=KnowledgeSyncSettingAPI.get_request(),
            responses=KnowledgeSyncSettingAPI.get_response(),
            tags=[_("Knowledge Base")],  # type: ignore
        )
        @has_permissions(
        ...
        @log(
            menu="Knowledge Base",
            operate="Update knowledge scheduled synchronization setting",
            get_operation_object=lambda r, keywords: get_knowledge_operation_object(keywords.get("knowledge_id")),
        )
        def put(self, request: Request, workspace_id: str, knowledge_id: str):
            return result.success(
                KnowledgeSyncSettingOperationSerializer(
                    data={"workspace_id": workspace_id, "knowledge_id": knowledge_id}
                ).update_setting(request.data)
            )

    class KnowledgeSyncLog(APIView):
        authentication_classes = [TokenAuth]

        @extend_schema(
            methods=["GET"],
            summary=_("Get knowledge synchronization logs"),
            description=_("Get knowledge synchronization logs"),
            operation_id=_("Get knowledge synchronization logs"),  # type: ignore
            parameters=KnowledgeSyncLogAPI.get_parameters(),
            responses=KnowledgeSyncLogAPI.get_response(),
            tags=[_("Knowledge Base")],  # type: ignore
        )
        @has_permissions(
            PermissionConstants.KNOWLEDGE_READ.get_workspace_knowledge_permission(),
        ...
        def get(
            ...
            return result.success(
                KnowledgeSyncLogQuerySerializer(data={"workspace_id": workspace_id, "knowledge_id": knowledge_id}).page(
    ...
    class ExportKnowledge(APIView):
        ...
        @log(
            menu="Knowledge Base",
            operate="Export knowledge bundle",
            get_operation_object=lambda r, keywords: get_knowledge_operation_object(keywords.get("knowledge_id")),
        )
        def get(self, request: Request, workspace_id: str, knowledge_id: str):
            return KnowledgeSerializer.Operate(
    ...
    class ImportKnowledge(APIView):
        ...
        @log(
            menu="Knowledge Base",
            operate="Import knowledge bundle",
        )
            ...
            return result.success(
                KnowledgeSerializer.ImportKnowledge(
                    data={
                        "workspace_id": workspace_id,
                        "user_id": request.user.id,
                        "folder_id": request.data.get("folder_id", workspace_id),
                    }
    ...
    class Model(APIView):
        ...
        def get(self, request: Request, workspace_id: str):
            return result.success(
                ModelSerializer.Query(data={"workspace_id": workspace_id, "model_type": "LLM"}).list(workspace_id, True)
            )
    ...
    class EmbeddingModel(APIView):
        ...
        def get(self, request: Request, workspace_id: str):
            return result.success(
                ModelSerializer.Query(data={"workspace_id": workspace_id, "model_type": "EMBEDDING"}).list(
                    workspace_id, True
                )
            )
...
class KnowledgeBaseView(APIView):
    ...
    def post(self, request: Request, workspace_id: str):
        return result.success(
            KnowledgeSerializer.Create(data={"user_id": request.user.id, "workspace_id": workspace_id}).save_base(
                request.data
            )
        )
...
class KnowledgeWebView(APIView):
    ...
    def post(self, request: Request, workspace_id: str):
        return result.success(
            KnowledgeSerializer.Create(data={"user_id": request.user.id, "workspace_id": workspace_id}).save_web(
                request.data
            )
        )
