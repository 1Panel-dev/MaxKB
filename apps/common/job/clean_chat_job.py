...
def clean_chat_log_job_lock():
    ...
    cutoff_dates = {app["id"]: now - datetime.timedelta(days=app["clean_time"] or 180) for app in applications}
    file_cutoff_dates = {
...
def delete_orphan_chats(orphan_chat_ids):
    ...
    if app_user_ids:
        ...
        remaining_keys = (
            ...
            .exclude(id__in=orphan_chat_ids)
            .values_list("application_id", "chat_user_id")
            .distinct()
        )
...
def clean_method(query_conditions, clean_log=True):
    ...
    while True:
        with transaction.atomic():
            ...
            if clean_log:
                ...
                updated_counts = (
                    ChatRecord.objects.filter(chat_id__in=chat_ids).values("chat_id").annotate(count=Count("id"))
                )
    ...
    if clean_log:
        orphan_chat_ids = list(Chat.objects.filter(chatrecord__isnull=True).values_list("id", flat=True))
