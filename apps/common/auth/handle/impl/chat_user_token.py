...
def get_auth(login_type, user_id, application_id):
    application_access_token_list = QuerySet(ApplicationAccessToken).filter(is_active=True)
    if login_type.upper() == str(Operate.ANNOTATION_AUTH):
        ...
        application_access_token_list = application_access_token_list.filter(
            authentication=True, authentication_value__type="password"
        )
    ...
    for application_access_token in application_access_token_list:
        ...
        if application_access_token.authentication:
            ...
            if authentication_value.get("type") == "login":
...
class ChatUserToken(AuthBaseHandle):
    ...
    def handle(self, request, token: str, get_token_details):
        ...
        _type = (
            ChatUserType.CHAT_USER if login_type_list.__contains__(login_type.upper()) else ChatUserType.ANONYMOUS_USER
        )
        auth = get_auth(login_type, user_id, application_id)
        ...
        if application_id:
            ...
            if not auth.permissions.get(f"{Group.CHAT_USER}:r:{application_id}"):
                raise AppUnauthorizedFailed(403, _("No permission to access"))
            return Principal(user_id, _type, application_id=application_id, profile=chat_user), auth
        return Principal(user_id, _type, profile=chat_user), auth
