...
def get_trigger_operation_object_batch(trigger_id_list):
    ...
    if trigger_model_list is not None:
        return {
            ...
            "trigger_list": [
                {"name": trigger_model.name, "type": trigger_model.type} for trigger_model in trigger_model_list
            ],
        }
...
class TriggerView(APIView):
    ...
    def post(self, request: Request, workspace_id: str):
        return result.success(
            TriggerSerializer(data={"workspace_id": workspace_id, "user_id": request.user.id}).insert(request.data)
        )
    ...
    class BatchDelete(APIView):
        ...
        def put(self, request: Request, workspace_id: str):
            return result.success(
                TriggerSerializer.Batch(data={"workspace_id": workspace_id, "user_id": request.user.id}).batch_delete(
                    request.data
                )
            )
    ...
    class BatchActivate(APIView):
        ...
        def put(self, request: Request, workspace_id: str):
            return result.success(
                TriggerSerializer.Batch(data={"workspace_id": workspace_id, "user_id": request.user.id}).batch_switch(
                    request.data
                )
            )
...
class TaskSourceTriggerView(APIView):
    ...
    @has_permissions(
        lambda r, kwargs: PermissionConstants[
            f"{kwargs.get('source_type')}_TRIGGER_CREATE"
        ]._build_workspace_permission(resource_id_key="source_id"),
        lambda r, kwargs: PermissionConstants[
            f"{kwargs.get('source_type')}_TRIGGER_CREATE"
        ...
        ViewPermission(
            ...
            [
                lambda r, kwargs: PermissionConstants[kwargs.get("source_type")]._build_workspace_permission(
                    resource_id_key="source_id"
                )(r, **kwargs)
            ],
    ...
    def post(self, request: Request, workspace_id: str, source_type: str, source_id: str):
        return result.success(
            TaskSourceTriggerSerializer(data={"workspace_id": workspace_id, "user_id": request.user.id}).insert(
                {
                    **request.data,
                    "source_id": source_id,
                    "workspace_id": workspace_id,
    ...
    @has_permissions(
        lambda r, kwargs: PermissionConstants[f"{kwargs.get('source_type')}_TRIGGER_READ"]._build_workspace_permission(
            resource_id_key="source_id"
        ),
        lambda r, kwargs: PermissionConstants[
            f"{kwargs.get('source_type')}_TRIGGER_READ"
        ].get_workspace_permission_workspace_manage_role(),
        RoleConstants.USER.get_workspace_role(),
        RoleConstants.WORKSPACE_MANAGE.get_workspace_role(),
    )
    ...
    class Operate(APIView):
        ...
        @has_permissions(
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source_type')}_TRIGGER_EDIT"
            ]._build_workspace_permission(resource_id_key="source_id"),
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source_type')}_TRIGGER_EDIT"
            ].get_workspace_permission_workspace_manage_role(),
        ...
        @has_permissions(
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source_type')}_TRIGGER_DELETE"
            ]._build_workspace_permission(resource_id_key="source_id"),
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source_type')}_TRIGGER_DELETE"
            ].get_workspace_permission_workspace_manage_role(),
