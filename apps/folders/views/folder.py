...
from common.auth.struct.aggregate_permission import ViewPermission, PermissionFunc
...
class FolderView(APIView):
    ...
    @has_permissions(
        lambda r, kwargs: PermissionConstants[f"{kwargs.get('source')}_FOLDER_CREATE"].get_workspace_permission(),
        lambda r, kwargs: PermissionConstants[
            f"{kwargs.get('source')}_FOLDER_CREATE"
        ].get_workspace_permission_workspace_manage_role(),
        ViewPermission(
            [RoleConstants.USER.get_workspace_role()],
            [
                lambda r, kwargs: Permission(
                    group=PermissionConstants[kwargs.get("source")].value.group,
                    sub_group=PermissionConstants[kwargs.get("source")].value.sub_group,
                    operate=PermissionConstants[kwargs.get("source")].value.operate,
                    bit_index=PermissionConstants[kwargs.get("source")].value.bit_index,
                    workspace_id=kwargs.get("workspace_id"),
                    resource_id=r.data.get("parent_id"),
                )
            ],
    ...
    def post(self, request: Request, workspace_id: str, source: str):
        return result.success(
            FolderSerializer.Create(
                data={"user_id": request.user.id, "source": source, "workspace_id": workspace_id}
            ).insert(request.data)
        )
    ...
    @has_permissions(
        lambda r, kwargs: PermissionConstants[f"{kwargs.get('source')}_FOLDER_READ"].get_workspace_permission(),
        lambda r, kwargs: PermissionConstants[f"{kwargs.get('source')}_READ"].get_workspace_permission(),
        RoleConstants.WORKSPACE_MANAGE.get_workspace_role(),
    )
    def get(self, request: Request, workspace_id: str, source: str):
        return result.success(
            FolderTreeSerializer(data={"workspace_id": workspace_id, "source": source}).get_folder_tree(
                request.user, request.query_params.get("name")
            )
        )
    ...
    class Operate(APIView):
        ...
        @has_permissions(
            lambda r, kwargs: PermissionConstants[f"{kwargs.get('source')}_FOLDER_EDIT"]._build_workspace_permission(
                resource_id_key="folder_id"
            ),
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source')}_FOLDER_EDIT"
            ].get_workspace_permission_workspace_manage_role(),
            ViewPermission(
                [RoleConstants.USER.get_workspace_role()],
                [
                    lambda r, kwargs: PermissionConstants[kwargs.get("source")]._build_workspace_permission(
                        resource_id_key="folder_id"
                    )(r, **kwargs)
                ],
                compare=CompareConstants.AND,
        ...
        @has_permissions(
            lambda r, kwargs: PermissionConstants[f"{kwargs.get('source')}_FOLDER_DELETE"]._build_workspace_permission(
                resource_id_key="folder_id"
            ),
            lambda r, kwargs: PermissionConstants[
                f"{kwargs.get('source')}_FOLDER_DELETE"
            ].get_workspace_permission_workspace_manage_role(),
            ViewPermission(
                [RoleConstants.USER.get_workspace_role()],
                [
