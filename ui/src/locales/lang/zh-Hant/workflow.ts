export default {
  ...
  setting: {
    ...
    defaultModelSettingTip: '節點選擇「預設模型」時,將使用以下配置。',
    defaultModelNotConfigured: '未設定預設模型',
    defaultModelUnavailable: '預設模型暫不可用',
    defaultModelRequired: '該類型預設模型未設定',
    defaultModelMissing: '{modelType}的預設模型未配置',
    applyToAll: '套用到所有節點',
    apply: '套用',
    applyToAllConfirmMsg: '將把所有節點的模型來源統一改為「預設模型」，會覆蓋它們目前的「自訂／引用變數」，請謹慎操作。',
    applyToAllSuccess: '已將 {count} 個節點設為預設模型',
    applyToAllNone: '已沒有需要修改的節點',
    defaultModelSettingUnsaved: '預設模型設定已修改，是否儲存修改？',
    saveChanges: '儲存修改',
    discardChanges: '不儲存',
    modelType: {
      LLM: 'AI 模型',
