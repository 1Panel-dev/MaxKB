export default {
  ...
  setting: {
    ...
    defaultModelSettingTip: '节点选择「默认模型」时,将使用以下配置。',
    defaultModelNotConfigured: '未配置默认模型',
    defaultModelUnavailable: '默认模型暂不可用',
    defaultModelRequired: '该类型默认模型未配置',
    defaultModelMissing: '{modelType}的默认模型未配置',
    applyToAll: '应用到所有节点',
    apply: '应用',
    applyToAllConfirmMsg: '将把所有节点的模型来源统一改为「默认模型」，会覆盖它们当前的「自定义／引用变量」，请谨慎操作。',
    applyToAllSuccess: '已将 {count} 个节点设为默认模型',
    applyToAllNone: '已没有需要修改的节点',
    defaultModelSettingUnsaved: '默认模型设置已修改，是否保存修改？',
    saveChanges: '保存修改',
    discardChanges: '不保存',
    modelType: {
      LLM: 'AI 模型',
