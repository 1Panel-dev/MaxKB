...
const getUsedByResources = (resource: ResourceType, resourceId: string, page: ParamsPage, query?: Dict<unknown>) => {
  return get<ResponsePage<RelatedResource>>(`${prefix}/resource_mapping/${resource}/${resourceId}/${page.currentPage}/${page.pageSize}`, query)
}

/** 获取当前资源依赖的资源。 */
const getUsingResources = (resource: ResourceType, resourceId: string, page: ParamsPage, query?: Dict<unknown>) => {
  return get<ResponsePage<RelatedResource>>(`${prefix}/mapping_resource/${resource}/${resourceId}/${page.currentPage}/${page.pageSize}`, query)
}
