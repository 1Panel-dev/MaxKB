...
/** 获取全部对话用户组。 */
function getChatUserGroups() {
  return get<ListItem[]>(prefix)
}

/** 创建或重命名对话用户组。 */
function postChatUserGroup(group: ChatUserGroupRequest) {
  return post<ChatUserGroupRequest, boolean>(prefix, group)
}

/** 删除对话用户组。 */
function deleteChatUserGroup(groupId: string) {
  return del<undefined, boolean>(`${prefix}/${groupId}`)
}

/** 获取用户组成员分页列表。 */
function getChatUserGroupMembers(groupId: string, page: ParamsPage, query?: RequestParams) {
...
/** 添加对话用户组成员。 */
function postChatUserGroupMembers(groupId: string, userIds: string[]) {
...
/** 移除对话用户组成员。 */
function postRemoveChatUserGroupMembers(groupId: string, relationIds: string[]) {
