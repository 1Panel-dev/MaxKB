...
const getUsedByResources = (resource: ResourceType, resourceId: string, page: ParamsPage, query?: Dict<unknown>) => {
  return get<ResponsePage<RelatedResource>>(
    `/workspace/${getWorkspaceId()}/resource_mapping/${resource}/${resourceId}/${page.currentPage}/${page.pageSize}`,
    query,
  )
}
...
const getUsingResources = (resource: ResourceType, resourceId: string, page: ParamsPage, query?: Dict<unknown>) => {
  return get<ResponsePage<RelatedResource>>(
    `/workspace/${getWorkspaceId()}/mapping_resource/${resource}/${resourceId}/${page.currentPage}/${page.pageSize}`,
    query,
  )
}
