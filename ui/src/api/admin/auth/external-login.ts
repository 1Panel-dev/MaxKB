...
/** 获取外部认证方式的跳转配置。 */
function getExternalAuthSetting(authType: string) {
  return get<ExternalAuthSetting>(`/login/auth/${authType}/detail`)
}

/** 获取扫码登录提供商配置。 */
function getQrCodeSources() {
...
/** 使用钉钉扫码授权码登录。 */
function getDingTalkCallback(code: string) {
  return get<LoginResponse>('/dingtalk', { code })
}

/** 使用钉钉客户端授权码登录。 */
function getDingTalkOauthCallback(code: string) {
  return get<LoginResponse>('/dingtalk/oauth2', { code })
}

/** 使用飞书客户端授权码登录。 */
function getLarkOauthCallback(code: string) {
