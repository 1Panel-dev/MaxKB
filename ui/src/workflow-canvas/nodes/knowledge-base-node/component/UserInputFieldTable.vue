<script setup lang="ts">
import { useTemplateRef } from 'vue'
import { cloneDeep } from 'lodash'
import { MsgError } from '@/utils/message'
import { dynamicFormTypeOptions, type FormField } from '@/components/mk-dynamics-form'
import type { KnowledgeFieldConfig } from '../types'
import UserFieldFormDialog from './UserFieldFormDialog.vue'
import TitleSettingDialog from './TitleSettingDialog.vue'

const fields = defineModel<FormField[]>({ required: true })
const config = defineModel<KnowledgeFieldConfig>('config', { required: true })
const fieldDialogRef = useTemplateRef<InstanceType<typeof UserFieldFormDialog>>('fieldDialogRef')
const titleDialogRef = useTemplateRef<InstanceType<typeof TitleSettingDialog>>('titleDialogRef')

// 表格只维护列表和重名检查，节点入口统一同步变量。
function handleOpenField(field?: FormField, index?: number) {
  fieldDialogRef.value?.open(field, index)
}
function handleOpenTitle() {
  titleDialogRef.value?.open(config.value)
}
function formatLabel(field: FormField) {
  return typeof field.label === 'string' ? field.label : (field.label?.label ?? field.field)
}
function formatDefaultValue(field: FormField) {
  if (field.input_type === 'PasswordInput') return '******'
  const value = field.default_value
  const values = Array.isArray(value) ? value : [value]
  if (field.option_list?.length) {
    return values
      .map((selected) => {
        const option = field.option_list?.find((option) => option.value === selected)
        return String(option?.label ?? option?.key ?? selected ?? '')
      })
      .join('、')
  }
  return Array.isArray(value) ? value.join('、') : String(value ?? '')
}

function saveField(data: FormField, index?: number) {
  if (fields.value.some((field, fieldIndex) => field.field === data.field && fieldIndex !== index)) {
    MsgError(`参数已存在：${data.field}`)
    return
  }
  const nextFields = cloneDeep(fields.value)
  if (index === undefined) nextFields.push(cloneDeep(data))
  else nextFields.splice(index, 1, cloneDeep(data))
  fields.value = nextFields
  fieldDialogRef.value?.close()
}

function deleteField(index: number) {
  fields.value = cloneDeep(fields.value.filter((_, fieldIndex) => fieldIndex !== index))
}

function saveTitle(data: KnowledgeFieldConfig) {
  config.value = cloneDeep(data)
  titleDialogRef.value?.close()
}
</script>

<template>
  <div class="w-full">
    <div class="flex-between gap-2">
      <span class="min-w-0 truncate" :title="config.title">{{ config.title }}</span>
      <div class="flex shrink-0 items-center">
        <!-- 设置标题 -->
        <el-button text type="primary" @click="handleOpenTitle"><MkIcon name="icon_setting" /></el-button>
        <!-- 添加参数 -->
        <el-button text type="primary" @click="handleOpenField()"><MkIcon name="icon_add_outlined" /></el-button>
      </div>
    </div>
    <MkTable v-if="fields.length" v-model:data="fields" sortable size="small" row-key="field" class="mt-2 border" :max-height="undefined">
      <el-table-column prop="field" label="参数" min-width="100">
        <template #default="{ row }">
          <span class="block truncate" :title="row.field">{{ row.field }}</span>
        </template>
      </el-table-column>
      <el-table-column label="显示名称" min-width="100">
        <template #default="{ row }">
          <span class="block truncate" :title="formatLabel(row)">{{ formatLabel(row) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="input_type" label="组件类型" width="95">
        <template #default="{ row }"
          ><el-tag type="info" size="small">{{
            dynamicFormTypeOptions.find(({ value }) => value === row.input_type)?.label ?? row.input_type
          }}</el-tag></template
        >
      </el-table-column>
      <el-table-column label="默认值" min-width="80">
        <template #default="{ row }">
          <span class="block truncate" :title="formatDefaultValue(row)">{{ formatDefaultValue(row) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="必填" width="55">
        <template #default="{ row }"><el-switch :model-value="Boolean(row.required)" disabled size="small" /></template>
      </el-table-column>
      <el-table-column label="操作" width="80">
        <template #default="{ row, $index }">
          <!-- 编辑参数 -->
          <el-button text type="primary" @click="handleOpenField(row, $index)"><MkIcon name="icon_edit_outlined" /></el-button>
          <!-- 删除参数 -->
          <el-button text type="primary" @click="deleteField($index)"><MkIcon name="icon_delete-trash_outlined" /></el-button>
        </template>
      </el-table-column>
    </MkTable>
  </div>
  <UserFieldFormDialog ref="fieldDialogRef" @submit="saveField" />
  <TitleSettingDialog ref="titleDialogRef" @submit="saveTitle" />
</template>
