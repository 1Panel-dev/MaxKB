...
const defaultForm: ImageGenerateNodeForm = {
  model_id: '',
...
const savedForm = model.properties.node_data as Partial<ImageGenerateNodeForm> | undefined
...
const modelSetting = computed(() => {
  const defaultModel = model.getDefaultModelConfig('TTI')
...
function updateNodeData(setting: Partial<ImageGenerateNodeForm>) {
  model.properties.node_data = { ...formData.value, ...setting }
}

function changeModelSource(source: ImageGenerateNodeForm['model_id_type']) {
...
function validateModel(_rule: unknown, _value: unknown, callback: (error?: Error) => void) {
  ...
  callback(
    modelSetting.value.model_id
      ? undefined
      : new Error(formData.value.model_id_type === 'default' ? '请在默认模型设置中选择图片生成模型' : '请选择图片生成模型'),
  )
}
        ...
        <el-form-item class="mk-hide-asterisk" :prop="modelFormProp" :rules="{ validator: validateModel, trigger: 'change' }">
          <template #label>
            <div class="flex-between">
              <span class="mk-required">图片生成模型</span>
