...
const modelSetting = computed(() => {
  const defaultModel = model.getDefaultModelConfig('TTV')
...
function changeModelSource(source: TextToVideoNodeForm['model_id_type']) {
...
function validateModel(_rule: unknown, _value: unknown, callback: (error?: Error) => void) {
  ...
  callback(
    modelSetting.value.model_id
      ? undefined
      : new Error(formData.value.model_id_type === 'default' ? '请在默认模型设置中选择文生视频模型' : '请选择文生视频模型'),
  )
}
        ...
        <el-form-item class="mk-hide-asterisk" :prop="modelFormProp" :rules="{ validator: validateModel, trigger: 'change' }">
          <template #label>
            <div class="flex-between">
              <span class="mk-required">文生视频模型</span>
