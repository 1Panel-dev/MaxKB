...
<template>
  <el-form-item class="small min-w-0 flex-10" :error="condition._fieldError">
    <FieldSelector v-model="condition.field" :left-options="leftOptions" placeholder="请选择变量" @change="handleFieldChange" />
  </el-form-item>

  <el-form-item class="small min-w-0 flex-6" :error="condition._compareError">
    <el-select v-model="condition.compare" clearable placeholder="请选择" @change="condition._compareError = ''">
      <el-option v-for="operation in condition._ops || compareList" :key="operation.value" :label="operation.label" :value="operation.value" />
    </el-select>
  </el-form-item>

  <el-form-item v-if="!['is_true', 'is_not_true'].includes(condition.compare)" class="small min-w-0 flex-6" :error="condition._valueError">
    <el-select
    ...
    <el-input v-else v-model="condition.value" placeholder="请输入选项值" @input="condition._valueError = ''" />
  </el-form-item>
</template>
