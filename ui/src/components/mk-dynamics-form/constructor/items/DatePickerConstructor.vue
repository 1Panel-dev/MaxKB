...
const getData = () => {
  return {
    ...
    attrs: { type: formValue.value.type, format: formValue.value.format, 'value-format': formValue.value.format },
...
<template>
  <div class="flex gap-4">
    <el-form-item class="min-w-0 flex-1" label="时间类型" required>
      <el-select v-model="formValue.type" placeholder="请选择时间类型" @change="handleDateTypeChange">
        <el-option v-for="inputTypeOption in dateTypeOptions" :key="inputTypeOption.value" :label="inputTypeOption.label" :value="inputTypeOption.value" />
      </el-select>
    </el-form-item>
    <el-form-item class="min-w-0 flex-1" label="格式" required>
      <el-select v-model="formValue.format" filterable default-first-option allow-create placeholder="请选择格式">
        <el-option
  ...
  <el-form-item class="mk-hide-asterisk" :required="formValue.required" prop="default_value" :rules="formValue.required ? [{ required: true, message: '请输入默认值' }] : []">
    ...
    <el-date-picker v-model="formValue.default_value" :type="formValue.type" placeholder="选择日期" :format="formValue.format" :value-format="formValue.format" class="w-full!" />
  </el-form-item>
</template>
