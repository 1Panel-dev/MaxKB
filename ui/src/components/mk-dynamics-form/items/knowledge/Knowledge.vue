...
const modelValueProxy = computed({
  get: () => props.modelValue.filter((id: string) => availableList.value.some((item) => item.id === id)) || [],
  set: (value: string[]) => {
...
<template>
  <div class="w-full">
    <el-select v-model="selectedIds" multiple v-bind="$attrs" class="w-full" placeholder="请选择知识库">
      ...
      <template #label="{ label, value }">
        <el-space :size="8">
          <KnowledgeIcon :type="availableList.find((item) => item.id === value)?.type" :size="14" style="--el-avatar-border-radius: 4px" />
          <span>{{ label }}</span>
        </el-space>
      </template>
    </el-select>
