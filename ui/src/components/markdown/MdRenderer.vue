<template>
  <template v-for="(item, index) in md_view_list" :key="index">
    <div
      ...
      @click="sendMessage ? sendMessage(item.content) : (content: string) => {}"
...
const props = withDefaults(
  defineProps<{
    ...
    sendMessage?: (q: string, other_params_data?: any) => void
  }>(),
  {
    source: ''
  }
...
const md_view_list = computed(() => {
  ...
  return split_form_rander(
    split_echarts_rander(split_html_rander(split_quick_question(split_md_img(result))))
  )
})
