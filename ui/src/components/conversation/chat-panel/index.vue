        ...
        <div class="operate flex-between">
          ...
          <div class="flex align-center">
            ...
            <el-tooltip effect="dark" placement="top" popper-class="upload-tooltip-width">
              ...
              <el-button
                text
                :disabled="loading || fileList.length >= maxFiles"
            ...
            <el-button v-else text class="sent-button stop-button" @click="$emit('stop')">
              <el-icon :size="20">
                <VideoPause />
              </el-icon>
            </el-button>
...
const { messages, loading, currentChatId, currentConversation } = store
...
// 同步发送锁：挡住连续回车 / 连点在 loading 置位前的重入
const sending = ref(false)

const placeholder = computed(() => {
  if (loading.value) return '正在回复中...'
  return '输入消息...'
})

const canSend = computed(
  () =>
    (question.value.content.trim() || fileList.value.length > 0) &&
    !loading.value &&
    !sending.value,
)
...
// ── 发送 ────────────────────────────────────────────────
const send = async () => {
  // 同步重入锁：必须在任何 await 之前判定并置位，挡住连续回车 / 连点。
