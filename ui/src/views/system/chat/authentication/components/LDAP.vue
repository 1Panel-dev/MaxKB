  ...
  return ChatUserAuthApi.getAuthSetting(form.auth_type)
    .then((setting) => {
      ...
      Object.assign(form, setting, {
        config: { ...form.config, ...setting.config, ldap_mapping: typeof ldapMapping === 'string' && ldapMapping ? JSON.stringify(JSON.parse(ldapMapping)) : ldapMapping },
      })
    })
...
<template>
  <el-form ref="authFormRef" v-loading="loading" class="max-w-200" :model="form" :rules="rules" label-position="top">
    ...
    <el-form-item label="用户过滤器" prop="config.ldap_filter"><el-input v-model="form.config.ldap_filter" placeholder="请输入用户过滤器" /></el-form-item>
