...
const rules = reactive<FormRules<AuthProviderSettingPayload>>({
  'config.idpMetaUrl': [{ whitespace: true, required: true, message: '请输入 Idp MetaData Url', trigger: 'blur' }],
  'config.privateKey': [{ whitespace: true, required: true, message: '请输入 SP Private Key', trigger: 'blur' }],
  'config.certificate': [{ whitespace: true, required: true, message: '请输入 SP Certificate', trigger: 'blur' }],
  'config.mapping': [{ whitespace: true, required: true, message: '请输入字段映射', trigger: 'blur' }],
  'config.spEntityId': [{ whitespace: true, required: true, message: '请输入 SP Entity Id', trigger: 'blur' }],
  'config.spAcs': [{ whitespace: true, required: true, message: '请输入 SP Ace', trigger: 'blur' }],
})
    ...
    .then((setting) => {
      ...
      Object.assign(form, setting, {
        config: {
          ...form.config,
          ...settingConfig,
          mapping: typeof mapping === 'string' && mapping ? JSON.stringify(JSON.parse(mapping)) : mapping || '',
