...
const form = reactive<AuthProviderSettingPayload>({ auth_type: LOGIN_METHOD.CAS, config: { ldpUri: '', validateUrl: '', redirectUrl: defaultRedirectUrl }, is_active: false })
  ...
  return AuthSettingApi.getAuthSetting(form.auth_type)
    .then((setting) => {
      ...
      Object.assign(form, setting, {
        config: { ...form.config, ...settingConfig, validateUrl: settingConfig.validateUrl || ldpUri, redirectUrl: settingConfig.redirectUrl || defaultRedirectUrl },
      })
    })
    .finally(() => {
      loading.value = false
...
<template>
  <el-form ref="authFormRef" v-loading="loading" class="max-w-200" :model="form" :rules="rules" label-position="top">
    <el-form-item label="ldpUri" prop="config.ldpUri"><el-input v-model="form.config.ldpUri" placeholder="请输入 ldpUri" /></el-form-item>
    <el-form-item label="验证地址" prop="config.validateUrl"><el-input v-model="form.config.validateUrl" placeholder="请输入验证地址" /></el-form-item>
