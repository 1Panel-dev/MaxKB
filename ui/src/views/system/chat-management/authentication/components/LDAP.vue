...
import ChatUserAuthApi from '@/api/admin/system/chat-management/chat-user-auth'
...
const rules = reactive<FormRules<AuthProviderSettingPayload>>({
  'config.ldap_server': [{ whitespace: true, required: true, message: '请输入服务器地址', trigger: 'blur' }],
  'config.base_dn': [{ whitespace: true, required: true, message: '请输入Bind DN', trigger: 'blur' }],
  'config.password': [{ required: true, message: '请输入密码', trigger: 'blur' }],
  'config.ou': [{ whitespace: true, required: true, message: '请输入用户目录（OU）', trigger: 'blur' }],
  'config.ldap_filter': [{ whitespace: true, required: true, message: '请输入用户过滤器', trigger: 'blur' }],
  'config.ldap_mapping': [{ whitespace: true, required: true, message: '请输入字段映射', trigger: 'blur' }],
})
    ...
    .then((setting) => {
      ...
      Object.assign(form, setting, {
        config: {
          ...form.config,
          ...setting.config,
          ldap_mapping: typeof ldapMapping === 'string' && ldapMapping ? JSON.stringify(JSON.parse(ldapMapping)) : ldapMapping,
        },
      })
    })
...
function submit(action: 'save' | 'test') {
  authFormRef.value?.validate((valid) => {
    if (!valid) return
    loading.value = true
    const request = action === 'test' ? ChatUserAuthApi.postAuthSettingConnection(form) : ChatUserAuthApi.putAuthSetting(form.auth_type, form)
    request.then(() => MsgSuccess(action === 'test' ? '连接成功' : '保存成功')).finally(() => (loading.value = false))
  })
}
...
<template>
  <el-form ref="authFormRef" v-loading="loading" class="max-w-200" :model="form" :rules="rules" label-position="top">
    ...
    <div>
      <!-- 保存 -->
      <el-button v-if="perm.system.chatAuth.edit()" type="primary" @click="submit('save')">保存</el-button>
      <!-- 测试连接 -->
