<template>
  <el-dialog
    :title="$t('views.application.editAIAvatar')"
...
const open = (data: any) => {
  ...
  if (data.avatar && data.avatar !== '') {
    ...
    avatarUrl.value = resetUrl(data.avatar)
  } else {
...
function submit() {
  if (radioType.value === 'default') {
    ...
    loadSharedApi({ type: 'application', systemType: apiType.value })
      .putApplicationAvatar(detail.value.id, null, loading)
      .then((res: any) => {
        emit('refresh', '')
    ...
    // 上传自定义头像
    loadSharedApi({ type: 'application', systemType: apiType.value })
      .putApplicationAvatar(detail.value.id, avatarFile.value.raw, loading)
      .then((res: any) => {
