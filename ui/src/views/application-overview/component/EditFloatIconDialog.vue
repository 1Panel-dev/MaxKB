<template>
  <el-dialog
    :title="$t('views.application.editFloatIcon')"
...
const open = (data: any) => {
  detail.value = data
  if (data.float_icon && data.float_icon !== '') {
    radioType.value = 'custom'
    iconUrl.value = resetUrl(data.float_icon)
  } else {
...
function submit() {
  if (radioType.value === 'default') {
    ...
    loadSharedApi({ type: 'application', systemType: apiType.value })
      .putApplicationFloatIcon(detail.value.id, null, loading)
      .then((res: any) => {
        emit('refresh', '')
    ...
    // 上传自定义图标
    loadSharedApi({ type: 'application', systemType: apiType.value })
      .putApplicationFloatIcon(detail.value.id, iconFile.value.raw, loading)
      .then((res: any) => {
