...
import type { ApplicationDetail, TriggerBodyField, TriggerTaskPayload, TriggerType } from '@/api/types'
    ...
    <MkCollapse v-model:expanded="expanded">
      <template #label>
        <div class="flex-align-center min-w-0 gap-2">
          <ApplicationIcon :icon="props.application?.icon" :size="20" class="shrink-0" />
          <span class="truncate" :title="props.application?.name || task.source_id">{{ props.application?.name || task.source_id }}</span>
        </div>
      </template>
      <TaskParameterForm
        ref="parameterRef"
        ...
        class="mt-2"
        :resource="{ type: RESOURCE_TYPE.APPLICATION, data: props.application }"
