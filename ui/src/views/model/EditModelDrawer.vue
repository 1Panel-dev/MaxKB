...
function open(provider: ModelProviderItem, model: ModelItem) {
  ...
  ModelApi.getModelDetail(model.id)
    .then((response) => {
...
function handleSubmit() {
  dynamicsFormRef.value?.validate().then(() => {
    loading.value = true
    return ModelApi.putModel(currentModelId.value, modelForm)
      .then(() => {
        MsgSuccess('修改成功')
        visible.value = false
        emit('refresh')
...
<template>
  <MkDrawer v-model="visible" :title="drawerTitle" direction="btt" @closed="resetData">
