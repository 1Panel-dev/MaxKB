...
const emit = defineEmits<{ refresh: [field: ToolInputField, index?: number] }>()
...
const formRules: FormRules<ToolInputField> = { name: [{ required: true, message: '请输入参数名称', trigger: 'blur' }] }
  ...
  <MkDialog v-model="visible" :title="isEdit ? '编辑参数' : '添加参数'" @closed="resetData">
    <el-form ref="formRef" :model="inputFieldForm" :rules="formRules" label-position="top" require-asterisk-position="right" @submit.prevent>
      <el-form-item label="参数名称" prop="name">
        <el-input v-model="inputFieldForm.name" maxlength="64" placeholder="请输入参数名称" @blur="inputFieldForm.name = inputFieldForm.name.trim()" />
      </el-form-item>
      <el-form-item label="数据类型">
        <el-select v-model="inputFieldForm.type" class="w-full">
          <el-option v-for="type in inputFieldTypeOptions" :key="type" :label="type" :value="type" />
        </el-select>
      </el-form-item>
      <el-form-item label="参数提示说明">
        <el-input v-model="inputFieldForm.desc" maxlength="128" show-word-limit placeholder="请输入参数提示说明" @blur="inputFieldForm.desc = inputFieldForm.desc?.trim()" />
      </el-form-item>
      ...
      <el-form-item label="是否必填" @click.prevent><el-switch v-model="inputFieldForm.is_required" /></el-form-item>
    </el-form>
    <template #footer>
